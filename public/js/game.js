...
tabCreate.addEventListener('click', () => { currentMode = 'create'; tabCreate.classList.add('active'); tabJ
oin.classList.remove('active'); panelCreate.classList.remove('hidden'); panelJoin.classList.add('hidden');
});
tabJoin.addEventListener('click', () => { currentMode = 'join'; tabJoin.classList.add('active'); tabCreate.
classList.remove('active'); panelJoin.classList.remove('hidden'); panelCreate.classList.add('hidden'); });
btnRed.addEventListener('click', () => { selectedTeam = 'red'; btnRed.classList.add('active'); btnBlue.clas
sList.remove('active'); });
btnBlue.addEventListener('click', () => { selectedTeam = 'blue'; btnBlue.classList.add('active'); btnRed.cl
...
li.innerText = p.name;
if (p.isAdmin) li.innerHTML += `<span class="admin-badge">HOST</span>`;
if (p.team === 'red') listRed.appendChild(li);
else listBlue.appendChild(li);
}
if (data.adminId === socket.id) {
btnStartMatch.classList.remove('hidden'); waitMessage.classList.add('hidden');
} else {
btnStartMatch.classList.add('hidden'); waitMessage.classList.remove('hidden');
}
});
btnStartMatch.addEventListener('click', () => { socket.emit('startMatchSignal'); });
socket.on('matchStarted', () => { lobbyContainer.classList.add('hidden'); gameContainer.classList.remove('h
idden'); setupInputListeners(); });
socket.on('gameState', (data) => {
clientPlayers = data.players; clientBall = data.ball; gameOverOverlay = data.gameOver;
if (data.score) scoreboard.innerHTML = `■ Red \${data.score.red} - \${data.score.blue} Blue ■`;
if (data.timeString) timerDisplay.innerHTML = `■■ \${data.timeString}`;
});
...
socket.on('goalScored', () => { goalOverlayActive = true; setTimeout(() => { goalOverlayActive = false; },
1500); });
socket.on('matchEnded', (winner) => { overlayText = winner === 'draw' ? "FIM DE JOGO: EMPATE!" : `FIM DE JO
GO: TIME \${winner.toUpperCase()} VENCEU!`; });
...
ctx.beginPath(); ctx.moveTo(canvas.width / 2, 0); ctx.lineTo(canvas.width / 2, canvas.height); ctx.stro
ke();
...
const posts = [{x: FIELD_MARGIN, y: GOAL_TOP}, {x: FIELD_MARGIN, y: GOAL_BOTTOM}, {x: canvas.width - FI
ELD_MARGIN, y: GOAL_TOP}, {x: canvas.width - FIELD_MARGIN, y: GOAL_BOTTOM}];
posts.forEach(post => { ctx.beginPath(); ctx.arc(post.x, post.y, 6, 0, Math.PI * 2); ctx.fill(); ctx.st
roke(); });
}
...
function drawBall() {
ctx.fillStyle = "#ffffff"; ctx.beginPath(); ctx.arc(clientBall.x, clientBall.y, clientBall.radius, 0, M
ath.PI * 2); ctx.fill(); ctx.lineWidth = 2; ctx.strokeStyle = '#000000'; ctx.stroke();
}
function drawOverlays() {
if (goalOverlayActive && !gameOverOverlay) {
ctx.fillStyle = "rgba(0, 0, 0, 0.6)"; ctx.fillRect(0, 0, canvas.width, canvas.height);
ctx.fillStyle = "#ffea00"; ctx.font = "bold 60px Arial"; ctx.textAlign = "center"; ctx.textBaseline
= "middle";
ctx.fillText("¡¡¡ GOOOL !!!", canvas.width / 2, canvas.height / 2);
}
if (gameOverOverlay) {
ctx.fillStyle = "rgba(0, 0, 0, 0.8)"; ctx.fillRect(0, 0, canvas.width, canvas.height);
ctx.fillStyle = "#ff3333"; ctx.font = "bold 40px Arial"; ctx.textAlign = "center"; ctx.textBaseline
= "middle";
ctx.fillText(overlayText, canvas.width / 2, canvas.height / 2);
}
}
function gameLoop() { drawField(); drawPlayers(); drawBall(); drawOverlays(); requestAnimationFrame(gameLoo
p); }
gameLoop();
