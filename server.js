...
io.on('connection', (socket) => {
    ...
    socket.on('joinOrCreateRoom', (data) => {
        ...
        let room = rooms[roomId]; const isRed = team === 'red';
        ...
        socket.emit('roomResponse', { success: true, roomId }); updateLobby(roomId);
    });
    ...
    socket.on('startMatchSignal', () => {
        if (currentRoomId && rooms[currentRoomId]) {
            ...
            if (socket.id === room.adminId && !room.matchActive) {
                room.matchActive = true; io.to(currentRoomId).emit('matchStarted');
            }
        }
    });
    ...
    socket.on('disconnect', () => {
        if (currentRoomId && rooms[currentRoomId]) {
            let room = rooms[currentRoomId]; delete room.players[socket.id];
            if (Object.keys(room.players).length === 0) { delete rooms[currentRoomId]; } 
            else {
...
let frameCount = 0;
setInterval(() => {
    frameCount++; const secondPassed = frameCount % 60 === 0;

    for (let roomId in rooms) {
        let room = rooms[roomId]; if (!room.matchActive) continue;
