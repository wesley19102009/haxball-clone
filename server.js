...
// Configuração completa do Socket.io permitindo CORS para rodar online sem bloqueios
...
if (!room) return;
io.to(roomId).emit('lobbyUpdate', { players: room.players, adminId: room.adminId });
}
...
if (mode === 'create') {
if (rooms[roomId]) return socket.emit('roomResponse', { success: false, message: "Esta sala já
existe!" });
rooms[roomId] = createRoomState(duration || 5, socket.id);
rooms[roomId].password = password || "";
} else {
if (!rooms[roomId]) return socket.emit('roomResponse', { success: false, message: "Sala não enc
ontrada!" });
...
if (room.gameOver) {
io.to(roomId).emit('gameState', { players: room.players, ball: room.ball, score: room.score, ti
meString: "00:00", gameOver: true });
continue;
}
...
if (room.timeLeft <= 0) {
room.gameOver = true;
room.winner = room.score.red > room.score.blue ? 'red' : (room.score.blue > room.score.red
? 'blue' : 'draw');
...
if (p.y - p.radius < 0) p.y = p.radius; if (p.y + p.radius > CANVAS_HEIGHT) p.y = CANVAS_HEIGHT
- p.radius;
...
if (room.ball.y + room.ball.radius > CANVAS_HEIGHT) { room.ball.y = room.ball.radius; room.ball.vy
*= -0.8; }
if (room.ball.x - room.ball.radius < FIELD_MARGIN) {
if (room.ball.y < GOAL_TOP || room.ball.y > GOAL_BOTTOM) { room.ball.x = FIELD_MARGIN + room.ba
ll.radius; room.ball.vx *= -0.8; }
else if (room.ball.x - room.ball.radius <= FIELD_MARGIN - GOAL_WIDTH) { room.score.blue += 1; r
esetMatch(room); io.to(roomId).emit('goalScored'); }
}
if (room.ball.x + room.ball.radius > CANVAS_WIDTH - FIELD_MARGIN) {
if (room.ball.y < GOAL_TOP || room.ball.y > GOAL_BOTTOM) { room.ball.x = CANVAS_WIDTH - FIELD_M
ARGIN - room.ball.radius; room.ball.vx *= -0.8; }
else if (room.ball.x + room.ball.radius >= CANVAS_WIDTH - FIELD_MARGIN + GOAL_WIDTH) { room.sco
re.red += 1; resetMatch(room); io.to(roomId).emit('goalScored'); }
}
io.to(roomId).emit('gameState', { players: room.players, ball: room.ball, score: room.score, timeSt
ring, gameOver: false });
}
}, 1000 / 60);
